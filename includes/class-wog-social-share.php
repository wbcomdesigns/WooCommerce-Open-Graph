 ...
 *
 * Handles social sharing buttons on product pages.
 */
class WOG_Social_Share {
	 ...
	 * @param array  $share_data The sharing data.
	 */
	...
	private function build_share_url( $platform, $share_data ) {
		...
		// Clean up the text data (but do not URL encode yet).
		$clean_title       = $this->clean_text_for_sharing( $raw_title );
		$clean_description = $this->clean_text_for_sharing( $raw_description );

		switch ( $platform ) {
			case 'facebook':
				// Facebook only needs the URL; it will fetch title and description from meta tags.
			...
			case 'pinterest':
				...
				$pin_description = $this->truncate_text( $pin_description, 500 );
			...
			case 'whatsapp':
				// WhatsApp mobile sharing format.
	 ...
	 * @return string
	 */
	public function debug_urls_shortcode( $atts ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found -- WordPress shortcode signature.
		...
		foreach ( $platforms as $platform ) {
			...
			$output   .= '<p><strong>' . esc_html( ucfirst( $platform ) ) . ':</strong><br>';
	...
	private function debug_share_urls( $product, $share_data ) {
		...
		foreach ( $platforms as $platform ) {
			$url = $this->build_share_url( $platform, $share_data );
			echo '<!-- ' . esc_html( $platform ) . ' URL: ' . esc_html( $url ) . " -->\n";
		}
		echo "<!-- END WOG DEBUG -->\n";
	}
}
